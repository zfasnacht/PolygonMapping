		...
		if tag == HC.DFTAG_VG:
			...
			for tags, refs in member_new:
				if tags == HC.DFTAG_NDG:
					sd_name, sd_ndims, sd_dims, sd_type, sd_nattrs=sd.select(sd.reftoindex(refs)).info()
					if sd_name == temp[-1]:
						return sd.select(sd.reftoindex(refs)).get()
				

#Reading data if hdf5 format
def read_hdf5(data_filename,path):
	#Reading the user given datafield from data file user has chosen 
	f = h5py.File(data_filename,path)
...
try:
	dataset = read_hdf5(data_filename,fieldname)
except:
